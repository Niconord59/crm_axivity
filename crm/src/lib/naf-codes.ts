/**
 * Table de correspondance des codes NAF (Nomenclature d'Activités Française)
...
export const NAF_CODES: Record<string, string> = {
  ...
  "23.52Z": "Fabrication de chite et plâtre",
 ...
 * @param code - NAF code (e.g., "62.01Z")
 * @returns The label or the code itself if not found
 */
export function getNafLabel(code: string | undefined | null): string | undefined {
  ...
  // Try exact match
  if (NAF_CODES[normalizedCode]) {
    return NAF_CODES[normalizedCode];
  }

  // Try with different formats (some APIs return without the letter suffix)
  const withZ = normalizedCode.endsWith("Z") ? normalizedCode : `${normalizedCode}Z`;
  if (NAF_CODES[withZ]) {
    return NAF_CODES[withZ];
  }

  // Try matching just the numeric part
  const numericPart = normalizedCode.replace(/[A-Z]$/, "");
  for (const [key, value] of Object.entries(NAF_CODES)) {
    if (key.startsWith(numericPart)) {
      return value;
    }
  }
